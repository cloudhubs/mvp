...
function countMegaservice(graphData: any, threshold: number) {
    return graphData.nodes.filter((node: any) => {
        let megaservice = node.patterns.find(
            (pattern: any) => pattern.type == "Megaservice"
        );
...
function countHighCoupling(graphData: any, threshold: number) {
    let len = graphData.nodes.filter((node: any) => {
        ...
        const neighborNodes = neighborLinks.map((link: any) => {
            if (link.source.nodeName == node.nodeName) {
                return link.target.nodeName;
            } else if (
                // In case of the above issue
                link.source == node.nodeName
            ) {
                return link.target;
            } else if (link.target.nodeName == node.nodeName) {
                return link.source.nodeName;
            } else {
                return link.source;
            }
        });
...
export { countCyclic, countHighCoupling, countBottleneck, countMegaservice };
