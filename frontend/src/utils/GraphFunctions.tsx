    ...
    switch (selectedAntiPattern) {
      case "Cyclic Dependency":
        const cyclic = node.patterns.find(
          (pattern: Antipattern) => pattern.type === "Cyclic Dependency"
        );
      ...
      case "Bottleneck":
        ...
        return bottlneck?.threshold > threshold ? IN_PATTERN : LIGHT_GRAY;
      case "Megaservice":
        const megaservice = node.patterns.find(
          (pattern: Antipattern) => pattern.type === "Megaservice"
        );
        return megaservice?.threshold > threshold ? IN_PATTERN : LIGHT_GRAY;
      case "High Coupling":
  ...
  if (antiPattern) {
    if (selectedAntiPattern == "coupling") {
      ...
      if (linkInAntiPattern(link, selectedAntiPattern)) {
        const color = IN_PATTERN.replace(`)`, `,0.99)`).replace("rgb", "rgba");
