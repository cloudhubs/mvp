...
/*const renderScene = graphRef.current.postProcessingComposer().passes[0];
        const bloomPass = new UnrealBloomPass(new THREE.Vector2(width, height), 1.5, 0.4, 0.85);
        bloomPass.strength = 3;
        bloomPass.radius = 1;
        bloomPass.threshold = 0.1;
        //bloomPass.clearColor = THREE.Color("rgba(0,0,0,0)")
        //graphRef.current.scene.background = null;
        //graphRef.current.postProcessingComposer().addPass(bloomPass);
        const bloomComposer = new EffectComposer(graphRef.current.renderer());
        bloomComposer.renderToScreen = true;
        bloomComposer.addPass( renderScene );
        bloomComposer.addPass( bloomPass );

        const renderPass = new RenderPass(graphRef.current.scene(), graphRef.current.camera())
        graphRef.current.renderer().setClearColor(0xff0000, 0);
        ...
        var finalComposer = new EffectComposer( graphRef.current.renderer() );
        finalComposer.addPass( renderScene );
        finalComposer.addPass( finalPass );

        //Graph.postProcessingComposer().addPass(bloomPass);
        //Graph.postProcessingComposer().addPass(renderScene);
        graphRef.current.postProcessingComposer().addPass(finalPass);*/